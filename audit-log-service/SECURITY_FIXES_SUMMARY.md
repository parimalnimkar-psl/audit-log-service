...
**Date:** August 18, 2026  
**Status:** Historical remediation summary. Not a current runtime test report.

> The curl snippets and line references below describe an earlier remediation attempt and must not be treated as evidence for the current tree. Current evidence is generated by `mvn clean verify`, the Surefire XML reports, JaCoCo XML, and the live checks documented in `README.md`.

## Current-state corrections

| Area | Current behavior | Evidence |
|---|---|---|
| H2 console | Protected by the local security chain and denied in the Keycloak profile; admin access is deployment-profile dependent | `SecurityConfig`, `KeycloakSecurityConfig` |
| Hash verification | Uses raw stored fields and database-compatible microsecond timestamps; escaped-value and SQL tamper regressions are tested | `AuditServiceTest`, `AuditPersistenceIntegrationTest` |
| User management | Uses `SCOPE_AUDIT_ADMIN`/`AUDIT_ADMIN` authorities; admin and reader tests exist | `UserController`, `UserControllerTest` |
| API controls | Export, page-size cap, request rate limiting, malformed-input and wrong-scope tests are present | `AuditController`, `ApiRateLimitFilterTest`, `AuditControllerTest` |
| External identity | Keycloak/OIDC is configured under the `keycloak` profile; no external Keycloak server was available for local execution | `application-keycloak.yml` |
...
# Historical example only. Current response contracts must be checked against the live API and tests.
