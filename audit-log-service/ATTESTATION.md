...
| Single-event reads are actor-scoped | [AuditController.java](src/main/java/com/example/audit/api/AuditController.java) `GET /audit/events/{id}`; admins may read any event, other readers are restricted to their authenticated actor and receive `404` otherwise | [AuditControllerTest.xml](target/surefire-reports/TEST-com.example.audit.api.AuditControllerTest.xml) and [AuditServiceTest.xml](target/surefire-reports/TEST-com.example.audit.service.AuditServiceTest.xml) | Verified |
...
| The automated validation gate passes | Maven configuration in [pom.xml](pom.xml) runs Surefire, JaCoCo, and the 70% line-coverage check | `mvn clean verify`; current Surefire XML totals: 101 tests, 0 failures, 0 errors, 0 skipped; current JaCoCo XML: 348/415 lines covered (83.9%), 86/122 branches covered (70.5%) | Verified |
